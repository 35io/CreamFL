...
class Node:
    ...
    def send_msg(self, message):
        ...
        print("curent key: ", self.node_id+"-"+ self.port)
        data = pickle.dumps({self.node_id+"-"+ self.port: message})
        for _ in range(len(self.peers)):
            self.dealer.send_multipart([data])
            self.logger.log(f"Node {self.node_id} sent message to peers {self.peers}")
    ...
    def recv_msg_loop(self):
        ...
        while self.running:
            try:
                ...
                self.logger.log(f"ZMQ or unpickle Error: {e}")
                break

    def get_from(self, from_node="", retries=1000, time_interal=1):
        # tag = from_node + "_" + key
        print("get from node", from_node)
...
class MMFL_Client(object):
    ...
    def load_dataset(self, args, is_vqa=False):
        ...
        #     # vqa_dataset = load_from_disk("/shared/project/xusong/data/vqa2/train")
        #     # test_dataset = load_from_disk("/shared/project/xusong/data/vqa2/test")
    ...
    def train(self, round_n, logger):
        self.cur_epoch = round_n
        logger.info(f"**** Client start round {round_n} and wait to receive global img-feature and txt-feature ..... ****")
        logger.info(f"**** 客户端执行第{round_n}轮训练开始，等待从服务端发送的全局知识. ****")
        # distill_index = data['msg']['distill_index']
        target_node = self.peers[0]
        print("target_node: ", target_node)
        global_items = self.client_node.get_from(from_node="0.0.0.0"+"-"+str(target_node[1]), time_interal=5)
        # print("current recv_cache: ", self.client_node.recv_dict)
        ...
        self.dataloaders_global = global_items["dataloaders_global"]
        self._dataloaders = global_items["_dataloaders"]

        # local training and generated representations
        img_vec, img_num = [], []
        txt_vec, txt_num = [], []
        print("==============", self.cur_trainers)
        
        # logger.info(f"Client received global img-feature {global_img_feature[:3]} and txt-feature {global_txt_feature[:3]}")
        logger.info(f"Client received global img-feature and txt-feature.")
        logger.info(f"Client start train on local datasets .....")
        logger.info(f"客户端接收到服务端的全局知识，开始进行本地训练.")
        ...
        for idx, trainer in enumerate(self.cur_trainers):
            ...
            _vec, i = trainer.generate_logits(self.dataloaders_global['train_subset_eval' + f'_{self.args.pub_data_num}'])  # {'img': img_vec, 'txt': txt_vec}
            # if not is_test:
        ...
        logger.info(f"Client local training ends.")
        logger.info(f"客户端完成本地训练，并将从本地数据提取的知识发送给服务端.")
        ...
        logger.info(f"**** Client send local knowlege feature to server.....")
        # logger.info(f"**** 客户端将.....")
        self.client_node.send_msg({"img_vec":img_vec, "txt_vec":txt_vec, "img_num":img_num, "txt_num":txt_num})
        logger.info(f"Client complete send local knowlege.")
        logger.info(f"客户端完成本地知识的发送，准备进行下一轮训练。")
...
class MMFL_Global(object):
    ...
    def train(self, round_n, logger):
        ...
        logger.info(f"Server train round: {round_n} and start extract knowledge from public datasets.")
        logger.info(f"服务端开始第{round_n}轮训练,并提取公共数据的知识.")
        self.cur_epoch = round_n
        ...
        # logger.info(f"Server send global img-feature {self.global_img_feature[:3]} and txt-feature {self.global_txt_feature[:3]} to clients.")
        logger.info(f"Server complete knonwledge extract and send global img-feature and txt-feature to clients.")
        logger.info(f"服务端完成知识提取并将提取特征发送给客户端.")

        self.global_node.send_msg({"global_img_feature":self.global_img_feature, "global_txt_feature":self.global_txt_feature, "distill_index": self.distill_index, "dataloaders_global": self.dataloaders_global, "_dataloaders": self._dataloaders})
        # self.global_node.send_msg({"dataloaders_global": self.dataloaders_global})
        logger.info(f"Server complete sending global img-feature and txt-feature to clients.")
        img_num = None
        img_vec = None
        txt_num = None
        txt_vec = None
        
        logger.info(f"***** Server wait to receive local img-feature and txt-feature from clients..... *****")
        logger.info(f"***** 服务端等待客户端训练完成并接收本地知识..... *****")

        for cur_peer in self.peers:
            # cur_node = cur_peer[0]
            cur_items = self.global_node.get_from(from_node="0.0.0.0"+"-"+str(cur_peer[1]), time_interal=20)
        ...
        if not self.args.disable_distill:
            print("**********start distill*************")
            logger.info(f"***** Server start to aggrate and distill between global and local knowledge..... *****")
            logger.info(f"***** 服务端接收完成所有客户端发送来的本地知识，开始进行知识融合和蒸馏..... *****")
            self.distill(round_n, img_vec, txt_vec, img_num, txt_num, self.distill_index)
            logger.info(f"***** Knowledge aggregate and distill is over. *****")
            logger.info(f"***** 服务端完成本轮的知识融合和蒸馏过程，准备下一轮迭代. *****")
        ...
        if self.vqa_engine is not None:
            ...
            test_scores = vqa_validation(100, self.vqa_engine.fusion_model, self.vqa_meta, self.vqa_test_loader)
        ...
        def save_model(type_name, score=score):
            ...
            if self.vqa_engine is not None:
                torch.save({'vqa': self.vqa_engine.fusion_model.state_dict(),
                            'score':score}, f'{prefix}_vqa_sl.pt')
            else:
                torch.save({'net': self.engine.model.state_dict(),
                            'score':score},  f'{prefix}_net_sl.pt')
